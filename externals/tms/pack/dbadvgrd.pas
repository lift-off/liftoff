...
uses
  ...
  Graphics, BaseGrid, AdvGrid, AdvObj, Dialogs
  {$IFDEF TMSDEBUG}
  , TMSUtil
  {$ENDIF}
  ;
...
begin
  ...
  if (FixedCols = 1) and ShowIndicator then
    ColWidths[0] := 12;
end;
...
begin
  ...
  begin
    if Fields.Count > 0 then
      ColCount := MinInt(1,FixedCols) + Fields.Count - NumHiddenColumns
    else if Assigned(FDataLink) then
    	if Assigned(FDataLink.DataSet) then
    		if FDataLink.DataSet.Active then
      		ColCount := MinInt(1,FixedCols) + VisibleFieldCount - NumHiddenColumns;
  end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if ((dt in [ftMemo,ftFmtMemo]) or ((dt = ftBlob) and ((Fields[FieldIdx] as TBlobField).BlobType = ftMemo))) and FShowMemoFields then
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if (dt in [ftMemo,ftFmtMemo]) and FShowMemoFields then
              begin
...
begin
  ...
  begin
    ...
    Fields.Add.Assign(Fields.Items[RealFrom - FixedCols]);
    Fields.Items[RealFrom - FixedCols].Free;

    AField := Fields.Insert(RealTo - FixedCols);
  ...
  begin
    cw := FMapping[FromIndex - FixedCols];
    for i := FromIndex to ColCount - 2 do
      FMapping[i - FixedCols] := FMapping[i + 1 - FixedCols];
    for i := ColCount - 1 downto ToIndex + 1 do
      FMapping[i - FixedCols] := FMapping[i - 1 - FixedCols];
    FMapping[ToIndex - FixedCols] := cw;
  end;
end;
