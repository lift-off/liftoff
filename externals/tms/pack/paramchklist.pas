...
uses
  ...
  StdCtrls, Dialogs, Menus, Spin, ComCtrls, ParHTML, PictureContainer, ParXPVS;
...
uses
  Consts, Shellapi, ShlObj, ActiveX
  {$IFDEF DELPHI4_LVL}
  ,ImgList
  {$ENDIF}
  ,CommCtrl;
...
begin
  ...
  HTMLDrawEx(Canvas,Items[index],hrect,FImages,pt.x,pt.y,fl,FHyperLink,FShadowOffset,
    False,False,False,False,(odSelected in State) and FShowSelection,fHover,True,FMouseDown,False,1.0, ParamColor,
    ...
    begin
      ...
      cbChecked:
        if AEnabled then
          ...
          DrawThemeBackground(HTheme,Canvas.Handle, BP_CHECKBOX,CBS_CHECKEDNORMAL or CBS_DISABLED,@DrawRect,nil);
      cbUnChecked:
        if AEnabled then
          DrawThemeBackground(HTheme,Canvas.Handle, BP_CHECKBOX,CBS_UNCHECKEDNORMAL,@DrawRect,nil)
        else
          DrawThemeBackground(HTheme,Canvas.Handle, BP_CHECKBOX,CBS_UNCHECKEDNORMAL or CBS_DISABLED,@DrawRect,nil);
      end;
...
begin
  ...
  begin
    ...
    if HTMLDrawEx(Canvas,self.items[res],r,fImages,X,Y,idx,-1,FShadowOffset,
      true,false,False,True,True,False,True,False,GetFocusRect,1.0,fParamColor,
...
procedure TParamCheckList.StartParamDir(param, curdir: string; hr: TRect);
var
  bi: TBrowseInfo;
  iIdList: PItemIDList;
