...
const
  ...
  BLD_VER = 0; // Build nr.
  DATE_VER = 'Feb, 2003'; // Month version
...
type
  ...
  TPropertyType = (ptInteger,ptFloat,ptText,ptColor,ptFont,ptBoolean,ptValues,
    ptIntSpin,ptTextButton,ptPropButton,ptDate,ptTime,ptCustom,ptValuesList,ptFixedColor,ptButton,ptPassword);
...
begin
  ...
  begin
    ...
    FControl.Assign((Source as TInspectorPanel).Control);
...
begin
  if Assigned(FInspectorDropTarget) then
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if FMouseDown and (FPanels[realtoppanel].Items[k].PropertyType = ptButton) then
          begin
            InvalidateItem(FPanels[realtoppanel],FPanels[realtoppanel].Items[k]);
            Exit;
          end;

          if FMouseDown and not FPanels[realtoppanel].Items[k].ReadOnly then
          begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if ( y >= j) and (y <= j + tempHeight) and
             (Cursor <> crHSplit) and (Button = mbLeft) then
          begin
          ...
          if ( y >= j) and (y <= j + tempHeight) and (x < FPanels[i].CaptionWidth + InspectorIndent) and
             (Cursor <> crHSplit) and (x > InspectorIndent) then
          begin
            ItemClicked(mbLeft,FPanels[i],FPanels[i].Items[k - 1]);
          end;

          if (y >= j) and (y <= j + tempHeight)
...
begin
  ...
  begin
    ...
    if IsHTML(HelpPanel.Text) then
      ...
      DrawText(Canvas.Handle,PChar(HelpPanel.Text),Length(HelpPanel.Text),R,DT_WORDBREAK);
  end;
end;
...
begin
  ...
  begin
    Canvas.Brush.Color := Panel.FSelectColor;
  ...
  if Panel.WordWrap then
    DrawText(canvas.Handle,PChar(Text),length(Text),lr,DT_CALCRECT)
  else
    DrawText(canvas.Handle,PChar(Text),length(Text),lr, DTSTYLE or DT_CALCRECT);
    ...
    begin
      ...
      begin
        if ((Panel.HoverIconColor <> clNone) and (Panel.HoverIconDownColor <> clNone)
          and PtInRect(MouseRect,pt) and (HasIcon or Panel.HoverFullWidth or not Item.Bitmap.Empty)) or Item.Down then
        begin
    ...
    begin
      ...
      begin
        if Item.InspectorPanel.WordWrap then
          DrawText(Canvas.Handle,PChar(text),Length(text),R,DT_LEFT or DT_WORDBREAK)
        else
...
begin
  ...
  begin
    ...
    begin
      if (APanel.Style = psProperties) and (AItem.PropertyType = ptButton) then
      begin
        if TabStop then
        begin
          SetFocus;
...
begin
  ...
  if (FInspectorColorCombo.ItemIndex = 40) and Assigned(FEditItem) then
  begin
  ...
  if Assigned(FOnEditColorChange) and Assigned(FEditItem) then
  begin
    FOnEditColorChange(Self,FEditItem.InspectorPanel, FEditItem,
