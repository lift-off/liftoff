...
uses
  stdctrls, graphics, wintypes, winprocs, messages, classes, controls, sysutils, forms, inifiles, registry;
...
begin
  ...
  allow := true;
  if assigned(fOnEditStart) then fOnEditStart(self, itemindex, allow);
...
begin
  ...
  begin
    if self.fELPersist.Storage = stInifile then
      ...
      Inifile := TInifile(TRegInifile.Create(self.fELPersist.Key));
...
begin
  ...
  begin
    if fELPersist.Storage = stInifile then
      ...
      Inifile := TInifile(TRegInifile.Create(fELPersist.Key));
