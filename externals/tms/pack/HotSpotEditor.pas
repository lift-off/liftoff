...
begin
  ...
  try
    ...
    Source.Canvas.StretchDraw(copyRect,FPicture.Bitmap);
...
begin
    ...
    if FPicture.Width+i<=0 then exit;
    if FPicture.Height+i<=0 then exit;
    if HotSpotsChanged then SwapHotSpots;
    FHotSpots.Assign(FHotSpots_org);
    CurrentDelta:=i;
    FHotSpots.ReScale(CurrentDelta);
...
begin
  if FSelectedItem>=FHotSpots.Count then FSelectedItem:=-1;
