...
type
  ...
  TFilterOperation = (foNONE, foAND, foXOR, foOR);
  ...
  TGridLook = (glStandard,glSoft,glClassic,glTMS,glXP);
  ...
  TCellType = (ctBitmap,ctIcon,ctNone,ctImageList,ctCheckBox,ctDataCheckBox,
    ...
    ctValue,ctProgress,ctComment,ctButton,ctBitButton,ctVirtCheckBox,ctProgressPie,ctSummary);
  ...
  published
    ...
    property ExpandOne: Boolean read FExpandOne write FExpandOne;
  ...
  published
    ...
    property DropDownAlwaysVisible: Boolean read FDropDownAlwaysVisible write SetDropDownAlwaysVisible;
    property FlatButton: Boolean read FFlatButton write SetFlatButton;
    property ProgressMarginX: Integer read FProgressMarginX write SetProgressMarginX;
    property ProgressMarginY: Integer read FProgressMarginY write SetProgressMarginY;
    property ProgressXP: Boolean read FProgressXP write SetProgressXP;
  end;
  ...
  published
    ...
    property MergeHeader: Boolean read FMergeHeader write FMergeHeader;
    property MergeSummary: Boolean read FMergeSummary write FMergeSummary;
    property Summary: Boolean read FSummary write FSummary;
    property SummaryColor: TColor read FSummaryColor write FSummaryColor;
    property SummaryColorTo: TColor read FSummaryColorTo write FSummaryColorTo;
    property SummaryTextColor: TColor read FSummaryTextColor write FSummaryTextColor;
  end;
  ...
  published
    ...
    property UseDisplayFont: Boolean read FUseDisplayFont write FUseDisplayFont;
  end;
  ...
  published
    ...
    property InitSortDirection: TSortDirection read FInitSortDirection write FInitSortDirection;
    ...
    property UndoSort: Boolean read FUndoSort write FUndoSort;
  end;
  ...
  protected
    ...
    procedure GetVisualProperties(ACol,ARow: Integer; var AState: TGridDrawState; Print, Select,Remap: Boolean;
      ABrush: TBrush; var AColorTo: TColor; AFont: TFont; var HA: TAlignment; var VA: TVAlignment; var WW: Boolean); override;
    ...
    procedure RemoveRowsInternal(RowIndex, RCount: Integer);
...
//    procedure PrivatePreviewRect(Preview: TPrintPreview; Gridrect:TGridRect; SelRows: Boolean);
    ...
    {
    procedure AddDetailRow(ARow,Height: Integer);
    procedure RemoveDetailRow(ARow: Integer);
    }
...
//    procedure AddCheckBoxEx(ACol,ARow: Integer;State,Data: Boolean;HAlign: TCellHalign;VAlign: TCellVAlign);
  ...
  published
    ...
    property Look: TGridLook read FLook write SetLook;
...
begin
  ...
  begin
    AState := [];
    GetVisualProperties(Col,Row,AState,False,False,True,Canvas.Brush,AColorTo,Canvas.Font,HAlign,VAlign,WW);
  ...
  begin
    if (self.SelLength = 0) and (self.SelStart = Length(Text)) and (Shift = []) then
      ...
      begin
        ...
        begin
          if (Col < ColCount - 1 - FixedRightCols) and HasStaticEdit(Col + 1, Row) then
          begin
...
begin
  ...
  if HasCellProperties(ACol,ARow) and (goEditing in Options) then
...
begin
  ...
  Result := (x < lvl * 16) and (x > (lvl - 1) * 16); 
end;
...
begin
  ...
  if pr <> -1 then
    CellProperties[0,ARow + 1].NodeLevel := lvl;
...
begin
  ...
  begin
    ...
    begin
      ...
      case Method of
      1:Floats[Colindex,k] := ColumnSum(Colindex,i,j - 1);
      2:Floats[Colindex,k] := ColumnAvg(Colindex,i,j - 1);
      3:Floats[Colindex,k] := ColumnMin(Colindex,i,j - 1);
      4:Floats[Colindex,k] := ColumnMax(Colindex,i,j - 1);
...
begin
  ...
  AState := [];
  GetVisualProperties(OCol,Row,AState,False,False,True,Canvas.Brush,AColorTo,Canvas.Font,HAlign,VAlign,WW);
  ...
  edComboEdit:
    begin
      ...
      EditCombo.Flat := Look in [glSoft,glTMS,glXP];
      EditCombo.Etched := Look in [glSoft,glTMS,glXP];
  ...
  edUniComboEdit,edUniComboList:
    begin
      ...
      ComboUni.Flat := Look in [glSoft,glTMS,glXP];
      ComboUni.Etched := Look in [glSoft,glTMS,glXP];
...
begin
  ...
  begin
    if IsWideCell(Col,Row) then
      Result := ValidateCellWide(WideCells[Col,Row])
    else
...
{
procedure TAdvStringGrid.ApplyFilterOld;
var
  i,j: Integer;
  hs,he: Integer;
...
begin
  ...
  GetVisualProperties(ACol,ARow,AState,False,True,True,Canvas.Brush,AColorTo,Canvas.Font,HAlign,VAlign,WW);
...
begin
  if not (Look in [glSoft,glTMS,glXP]) then
    Exit;
...
var
  ...
  begin
    ...
    begin
      ...
      Canvas.MoveTo(tr.Left - 1,tr.Top);
      Canvas.LineTo(tr.Left - 1,tr.Bottom);
    end;
  ...
  begin
    ...
    begin
      ...
      if Assigned(sl) then
      begin
        if (gdSelected in AState) and ShowSelection then
  ...
  begin
    ...
    ctProgressPie:
    begin
      DrawProgressPie(Canvas,Rect(TgtRect.left,TgtRect.Top,TgtRect.left+20,TgtRect.Top+20),TColor(CellGraphic.CellBitmap),CellGraphic.CellAngle);
    end;
    ...
    begin
      ...
      begin
        if CellGraphic.CellBoolean then
          DrawProgressLin(Canvas,r,TColor(CellGraphic.CellBitmap),TColor(CellGraphic.CellIndex),
            TColor(CellGraphic.CellIcon),TColor(CellGraphic.CellAngle),Ints[ACol,ARow],ControlLook.ProgressMarginX,ControlLook.ProgressMarginY)
        else
          DrawProgressLin(Canvas,r,TColor(CellGraphic.CellBitmap),TColor(CellGraphic.CellBitmap) xor $FFFFFF,
            TColor(CellGraphic.CellIcon),TColor(CellGraphic.CellIcon) xor $FFFFFF,Ints[ACol,ARow],ControlLook.ProgressMarginX,ControlLook.ProgressMarginY);
      end;      
    end;
    ...
    begin
      ...
      begin
        ...
        if ACol < FixedCols then
          r.Left := r.Left - 1;
        Canvas.MoveTo(r.Left + CellNode.NodeIndent div 2,r.Top + 6);
        Canvas.LineTo(r.Left + CellNode.NodeIndent div 2,r.Bottom);
      end;
  ...
  begin
    ...
    if ((gdSelected in Astate) or
       (RowSelect[RRow] and (goRowSelect in Options) and (ACol >= FixedCols))) and not
...
begin
  ...
  GetVisualProperties(OCol,ARow,AState,False,False,True,Canvas.Brush,AColorTo,Canvas.Font,HAlignment,VAlign,WW);
  ...
  begin
    DrawGradient(Canvas, FTMSGradFrom,FTMSGradTo,10,ARect,False);
  end;
  ...
  begin
    GetCellEditor(OCol,ARow,AEditor);

    if (AEditor in [edComboEdit,edComboList]) then
    begin
      DrawComboButton(Canvas,Self.Handle, ARect,FIsWinXP,false);
  ...
  begin
    ...
    begin
      ...
      Anchor := ClassName  + ' trial version ' + GetVersionString; {+ ' © 2003 http://www.tmssoftware.com';}
  ...
  begin
    ...
    if (lvl > 0) then
    begin
      Canvas.MoveTo(ARect.Left - (CellNode.NodeIndent div 2),ARect.Top + (ARect.Bottom - ARect.Top) shr 1);
      Canvas.LineTo(ARect.Right,ARect.Top + (ARect.Bottom - ARect.Top) shr 1);
    end;

    case GetNodeSpanType(ARow) of
    1:begin
        Canvas.MoveTo(ARect.Left - CellNode.NodeIndent div 2,ARect.Top);
        Canvas.LineTo(ARect.Left - CellNode.NodeIndent div 2,ARect.Top + (ARect.Bottom - ARect.Top) shr 1);
      end;
    2:begin
        Canvas.MoveTo(ARect.Left - CellNode.NodeIndent div 2, ARect.Top );
        Canvas.LineTo(ARect.Left - CellNode.NodeIndent div 2, ARect.Bottom + 4);
      end;
    end;
    ...
    begin
      ...
      begin
        Canvas.MoveTo(ARect.Left - (CellNode.NodeIndent div 2) - CellNode.NodeIndent * vpos, ARect.Top );
        Canvas.LineTo(ARect.Left - (CellNode.NodeIndent div 2) - CellNode.NodeIndent * vpos, ARect.Top + (ARect.Bottom - ARect.Top) shr 1);

        Canvas.LineTo(ARect.Left - (CellNode.NodeIndent div 2) - CellNode.NodeIndent * vpos, ARect.Bottom + 4);
      end;
    end;
...
begin
  ...
  begin
    ...
    if Assigned(UndoRedo) then
      UndoRedo.RegisterChange(ROldCol,FOldRow,OriginalCellValue,Value);
...
begin
  ...
  if FMouseActions.RangeSelectAndEdit and not HasStaticEdit(Col,Row) and IsEditable(rm,Row) then
  begin
...
begin
  ...
  begin
    ...
    begin
      nc := Col;
      while (nc > FixedCols + 1) and IsFixed(nc - 1,Row) do
      begin
        dec(nc);
      end;
    ...
    begin
      nr := Row;
      while (nr > FixedRows + 1) and IsFixed(Col,nr - 1) do
      begin
        dec(nr);
      end;
  ...
  begin
    ...
    begin
      Selection := TGridRect(Rect(FixedColsVis,FixedRows,
...
begin
  ...
  begin
    ...
    ColWidths[FSizeFixedCol] := Msg.XPos - offs;
  end;
...
begin
  ...
  begin
    ...
    if (x = 0) and (FNumNodes > 0) and not MouseActions.RowSelect then
      Exit;
  ...
  if (x < ColCount) and (y < RowCount) and
     (x >= FixedCols) and ((y >= FixedRows) or MouseActions.CheckAllCheck) and
     (CanEdit) and HasCheckBox(rx,y) and IsInCheckBox(rx,y,Msg.XPos,Msg.YPos) then
  begin
    ...
    begin
      for hl := FixedRows to RowCount - 1 do
        if HasCheckBox(rx,hl) then
          SetCheckBoxState(rx,hl,chk);
    end;
...
begin
  ...
  repeat
    while (FUnSortRowXRef.Items[i] < FUnSortRowXRef[m]) and (i < Right) do Inc(i);
    while (FUnSortRowXRef.Items[j] > FUnSortRowXRef[m]) and (j > Left) do Dec(j);
...
procedure TAdvStringGrid.QUnSort;
  // what needs to be done here is a quicksort based on the sortrowxref
var
  i: Integer;
begin
  ...
  QuickSortRowsRef(0,1,RowCount - 1);
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          GetVisualProperties(i,0,AState,False,False,True,Canvas.Brush,AColorTo,Canvas.Font,HAlign,VAlign,WW);
    ...
    begin
      ...
      begin
        ...
        GetVisualProperties(j,i,AState,False,False,True,Canvas.Brush,AColorTo,Canvas.Font,HAlign,VAlign,WW);
  ...
  begin
    for j := Selection.Left to Selection.Right do
      for k := Selection.Top to Selection.Bottom do
        if IsEditable(RemapCol(j),k) then
        begin
          Allow := true;
...
begin
  ...
  for i := FromRow to ToRow do
    if not ((IsNode(i) or IsSummary(i)) and (GroupColumn <> -1)) then
...
var
  ...
  begin
    ...
    GetVisualProperties(Col,Row,AState,True,False,False,OldBrush,AColorTo,NewFont,HAlign,VAlign,WW);
 ...
 {$ENDIF}
  ...
  begin
    ...
    begin
      ...
      GetVisualProperties(cn,ARow,AState,True,False,False,NewBrush,AColorTo,NewFont,HAlign,VAlign,WW);
              ...
              begin
                ...
                taLeftJustify:
                  begin
                    ...
                    DrawStyle := DT_LEFT or DT_SINGLELINE or DT_EDITCONTROL;
                  end;
                taRightJustify:
                  begin
                    tp.Left := tr.Left;
                    tp.Right := tr.Right - fnthspace;
                    DrawStyle := DT_RIGHT or DT_SINGLELINE or DT_EDITCONTROL;
                  end;
                taCenter:
                  begin
                    tp.Left := tr.Left + fnthspace;
                    tp.Right := tr.Right - fnthspace;
                    DrawStyle := DT_CENTER or DT_SINGLELINE or DT_EDITCONTROL;
                  end;
                end;
...
begin
  ...
  begin
    ...
    if (MouseActions.FixedColsEdit = fceLeftClick) and (Button = mbLeft) and (r < FixedCols) then
    begin
      StartFixedEdit(c,r);
      Exit;
    end;  
    ...
    if (MouseActions.FixedColsEdit = fceLeftClick) and (Button = mbRight) and (r < FixedCols) then
    begin
      StartFixedEdit(c,r);
      Exit;
    end;
...
begin
  ...
  begin
    for j := FixedCols to ColCount do
      ClearPropCell(j,i);
  end
  else
  begin
...
begin
  ...
  if Allow then
    ...
    begin
      ...
      if Key in [VK_DOWN,VK_UP,VK_TAB] then
        KeyDown(Key,Shift);
    end
  else
    inherited;
...
begin
  {$IFDEF DELPHI4_LVL}
  Result := DecodeWideStr(GridCells[i,j]);
  {$ENDIF}
end;
